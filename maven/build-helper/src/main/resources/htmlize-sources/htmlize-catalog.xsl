...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template match="cat:uri" mode="serialize">
		<span about="{pf:relativize-uri(resolve-uri(@uri,base-uri(/*)),$output-uri)}">
			<xsl:next-match/>
		</span>
	</xsl:template>
	
	<xsl:template match="cat:uri/@name" mode="attribute-value">
		<span property="dp2:id">
			<xsl:value-of select="."/>
		</span>
	</xsl:template>

	<xsl:template match="cat:uri/@uri" mode="attribute-value">
