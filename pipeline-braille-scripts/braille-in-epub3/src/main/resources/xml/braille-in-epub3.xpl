...
<p:declare-step type="px:braille-in-epub3" version="1.0"
    ...
    <px:fileset-create name="target.base.fileset">
        <p:with-option name="base" select="concat($output-dir,'/',replace($source,'^.*/(([^/]+)\.epub|([^/]+))$','$2$3_braille.epub'),'!/')"/>
...
<metadata xmlns:dcterms="http://purl.org/dc/terms/"/></p:inline>
    ...
    <p:for-each name="braille-rendition.html">
        ...
        <p:for-each name="resource-map">
            ...
            <p:xslt>
                <p:input port="source">
                    <p:pipe step="resource-map" port="current"/>
                    <p:pipe step="default-rendition.package-document" port="result"/>
                    <p:pipe step="braille-rendition.package-document" port="result"/>
                </p:input>
                <p:input port="stylesheet">
                    <p:document href="resource-map-as-nav.xsl"/>
                </p:input>
